...
export default function CheckoutSuccessPage() {
  ...
  const getDeliveryDate = () => {
    const isExpress = checkoutState.deliveryMethod === "express";
      ...
      <div className="max-w-2xl w-full mx-auto px-4 text-center">
        ...
        <div className="bg-white dark:bg-[#1E1E1E] border border-[var(--ag-gray-200)] dark:border-neutral-850 rounded-[var(--radius-2xl)] p-6 shadow-sm text-left space-y-5">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 border-b border-[var(--ag-gray-100)] dark:border-neutral-800 pb-4">
            <div>
              <span className="text-[10px] font-black text-[var(--ag-gray-500)] uppercase tracking-wider block">Order Reference ID</span>
              <span className="text-base font-black text-[var(--ag-dark)] dark:text-white tracking-tight">{orderId || "KP-481923"}</span>
            </div>
            <div className="sm:text-right">
              <span className="text-[10px] font-black text-[var(--ag-gray-500)] uppercase tracking-wider block">Estimated Delivery</span>
          ...
          {placedAddress && (
            <div className="flex gap-3 text-xs font-semibold text-[var(--ag-gray-800)] dark:text-neutral-300">
              <MapPin size={16} className="text-[var(--ag-red)] shrink-0 mt-0.5" />
              <div>
                <span className="font-black text-[var(--ag-dark)] dark:text-white block mb-0.5">Shipping to:</span>
                <p>{placedAddress.name} ({placedAddress.mobile})</p>
                <p className="text-[var(--ag-gray-500)] mt-0.5">{placedAddress.address}</p>
              </div>
            </div>
          )}
          ...
          {placedItems.length > 0 && (
            <div className="border-t border-[var(--ag-gray-100)] dark:border-neutral-800 pt-4">
              <span className="text-[10px] font-black text-[var(--ag-gray-500)] uppercase tracking-wider block mb-3">Items Purchased</span>
              <div className="divide-y divide-[var(--ag-gray-100)] dark:divide-neutral-850 max-h-[140px] overflow-y-auto pr-1">
                {placedItems.map((item) => (
                  <div key={item.id} className="flex justify-between items-center py-2 first:pt-0 last:pb-0 text-xs font-semibold">
                    <span className="text-[var(--ag-dark)] dark:text-white truncate max-w-[70%]">
                      {item.name} <span className="text-[var(--ag-gray-500)]">× {item.quantity}</span>
                    </span>
                    <span className="text-[var(--ag-dark)] dark:text-white font-bold">₹{item.price * item.quantity}</span>
                  </div>
                ))}
              </div>
            </div>
